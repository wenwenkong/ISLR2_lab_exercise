...
## Questions:
1. In the multiple regression model including qualitative predictors, why we don't see the parameters for `ShelveLocBad`?
...
The `vif()` function, part of the `car` package, can be used to compute variance inflation factors. Most VIF's are low to moderate for this data. The `car` package is not part of the base `R` installation so it must be downloaded the first time you use it via the `install.packages()` function in `R`.
